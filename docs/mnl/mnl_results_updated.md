...
### β 계수 (K1 사양 — 2026-03-21)

| Feature | β | t-stat | Sig | 비고 |
|---------|---|--------|-----|------|
| Bus IVT (min) | -0.0108 | -100.7 | *** | 모드별 분리 |
| Train IVT (min) | -0.0122 | -86.0 | *** | 모드별 분리 |
| ln(1+Access walk min) | -2.942 | -1658.8 | *** | 비선형 (log) |
| ln(1+Egress walk min) | -1.733 | -1549.9 | *** | 비선형 (log) |
| Transfer walk (min) | -0.0104 | -19.7 | *** | 선형 유지 |
| Transfers | -3.706 | -1912.1 | *** | |
| Fare (1000won) | -0.362 | -259.8 | *** | 차등요금 반영 |
| Has bus | -1.952 | -908.3 | *** | |
| Has train | +3.286 | +1137.0 | *** | |
| Has GTX | +0.651 | +57.0 | *** | |

**제거된 변수**: wait_time (구조적 식별 불가), total_distance (IVT와 r=0.882 공선성)

### 성능 평가

| 지표 | K1 (2026-03-21) | 이전 (2026-03-17) | 변화 |
|------|----------------|-------------------|------|
| Test ρ² | **0.5087** | 0.4868 | **+2.2%p** |
| Top-1 (FPR) | **72.16%** | 70.90% | **+1.3%p** |
| RMSE | **0.2467** | 0.2553 | **-0.009** |
| Bounded vars | **없음** | wait_time | **해소** |
...
| 변경 | 이유 |
|------|------|
| IVT → bus_ivt + train_ivt | 모드별 다른 가치 (train IVT가 1.1x) |
...
| fare 복원 | 버스유형별 차등요금 반영 → β=-0.362 활성화 |

## 4. DTUMOS 파라미터 매핑

| DTUMOS Param | K1 Estimated | 이전 | Default |
|--------------|-------------|------|---------|
| beta_time (bus) | -0.0108 | -0.007 | -0.0500 |
| beta_time (train) | -0.0122 | -- | -0.0500 |
| beta_cost | -0.000362/won | 0.000 | -0.0003 |
| beta_wait | -- (제거) | 0.000 | -0.0800 |
| beta_walk | log 사양 (직접 매핑 불가) | -0.454 | -0.1000 |
| beta_transfer | -3.706 | -3.657 | -0.3000 |
